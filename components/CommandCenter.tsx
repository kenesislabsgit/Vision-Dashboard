...
import { MOCK_INCIDENTS, MOCK_TIER_1_KPIS, MOCK_TIER_2_KPIS } from '../constants';
...
// --- Improved Generator for Smooth Sparklines ---
const generateSparklineData = (trend: 'up' | 'down' | 'neutral', label: string) => {
    // Seed random based on label length to keep it consistent between renders
    const seed = label.length; 
    const points = 20;
    
    return Array.from({ length: points }, (_, i) => {
        // Base sine wave for organic look
        ...
        // Calculate final value, ensure positive
...
// --- Component: Premium KPI Card ---
const KPICard: React.FC<{ kpi: KPI }> = ({ kpi }) => {
    // Logic to determine if 'Up' is good or bad
    ...
    // Color Palette
    // Critical = Red, Good Trend = Green, Bad Trend/Warning = Orange, Neutral = Gray
    let chartColor = '#6B7280'; // Default Gray
    if (isCritical) {
        chartColor = '#EF4444'; // Red-500
    } else if (isTrendGood) {
        chartColor = '#10B981'; // Emerald-500
    } else if (kpi.trend !== 'neutral') {
    ...
    return (
        <div className="relative overflow-hidden bg-white border border-gray-100 rounded-2xl shadow-card hover:shadow-card-hover transition-all duration-300 group h-44 flex flex-col justify-between p-6">
            ...
            {/* Chart Layer (Absolute Background) */}
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-0.5">
                    <span className={`text-sm font-bold truncate ${isActionRequired ? 'text-charcoal' : 'text-gray-600'}`}>
                        {incident.title}
                    </span>
                    <span className="text-xs font-mono font-medium text-gray-400 bg-white px-1.5 py-0.5 rounded border border-gray-100 shadow-sm">
                        {timeSince(incident.timestamp)}
                    </span>
                </div>
                <p className="text-xs text-gray-500 line-clamp-1 mb-2">{incident.description}</p>
                <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase bg-gray-100 px-1.5 py-0.5 rounded">
                        {incident.siteId}
                    </span>
                    {isActionRequired && (
                        <span className="text-[10px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded uppercase">Action Req</span>
                    )}
                </div>
            </div>
...
// --- Component: Digital Twin View (Replaces Live Feed) ---
        ...
        <div className="relative aspect-video bg-charcoal rounded-xl overflow-hidden shadow-card-dark group border border-gray-800 flex items-center justify-center">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                ...
                <div className="flex gap-1">
                    <button className="p-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded border border-gray-700 transition-colors">
            ...
            {/* Timestamp Scrubber replacement */}
...
// --- Component: Mission Control Workspace ---
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-white animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="px-6 lg:px-8 py-6 border-b border-gray-100 flex flex-col lg:flex-row justify-between items-start gap-4 flex-shrink-0 bg-white z-20">
                <div>
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                        <div className="px-2.5 py-1 bg-charcoal text-white text-xs font-mono rounded">
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-8 custom-scrollbar">
                {/* 1. Evidence Player & Analysis Grid */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Evidence - Spans 2 cols on Desktop */}
                    ...
                    {/* 2. AI Reasoning Engine - Spans 1 col */}
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                                        <div 
                                            className="bg-charcoal h-full rounded-full transition-all duration-1000" 
                                ...
                                <div className="p-3 bg-white rounded-lg border border-gray-200">
                                    ...
                                    <p className="text-sm font-medium text-charcoal leading-snug">
                                        {incident.rootCauseHypothesis || 'Analyzing sensor fusion patterns...'}
                                    </p>
                                </div>
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
                ...
                <div className="w-full lg:w-96 bg-white rounded-2xl shadow-card border border-gray-100 flex flex-col overflow-hidden flex-shrink-0 lg:h-full h-96">
                    <div className="p-4 border-b border-gray-100 bg-white flex justify-between items-center z-10 shadow-sm">
                        ...
                        <span className="bg-charcoal text-white text-[10px] px-2 py-0.5 rounded-full font-bold">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        ...
                        {activeIncidents.map(incident => (
                            <QueueItem 
                                key={incident.id} 
                                incident={incident} 
                                isSelected={selectedEventId === incident.id}
                                onSelect={() => setSelectedEventId(incident.id)}
                            />
                        ))}
